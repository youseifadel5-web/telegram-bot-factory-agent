...
def _public_base(request: Request) -> str:
    ...
    API_PUBLIC_BASE_URL wins in production. Otherwise use proxy headers (common
    on Render/Railway/Fly/VPS reverse proxies) and finally the request origin.
    This keeps localhost development working without hard-coding localhost into
    links returned to a remote phone.
    """
    configured = os.getenv("API_PUBLIC_BASE_URL", "").strip().rstrip("/")
    if configured:
        return configured
    ...
    if request.url.scheme in ("http", "https") and request.url.netloc:
...
async def stream(content_id: str, request: Request, episode_id: Optional[str] = None, authorization: Optional[str] = Header(default=None)):
    ...
    return {"success": True, "stream": {
        "url": f"{_public_base(request)}/api/stream/media/{token}",
    ...
    if "mpegurl" in ct or url.lower().split("?")[0].endswith((".m3u8", ".m3u")):
        ...
        def issue(u):
            ...
            tok = AUTH.issue_proxy_token(row["telegram_id"], parent_hash, absolute,
                                         row["expires_at"])
            return f"{_public_base(request)}/api/stream/proxy/{tok}"
        out = []
