...
_configured_admin = str(_c("ADMIN_ID", "")).strip()
try:
    ADMIN_ID = int(_ADMIN_ID_ENV or _configured_admin or "0")
except ValueError:
    ADMIN_ID = 0
try:
    ADMIN_IDS = [ADMIN_ID] if ADMIN_ID > 0 else [int(x) for x in _c("ADMIN_IDS", []) if str(x).isdigit() and int(x) > 0]
        ...
        try:
            ...
            for q, y, lang in attempts:
                ...
                if out.get("poster") or out.get("overview"):
                    break
            # تفاصيل العمل من endpoint التفاصيل لتحسين المدة والبيانات.
            if out.get("id"):
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        if not runtime and d.get("episode_run_time"):
                            runtime = (d.get("episode_run_time") or [None])[0]
                        if runtime:
                            out["runtime"] = runtime
                        out["title_ar"] = d.get("title") or d.get("name") or out.get("title")
                        out["original_title"] = d.get("original_title") or d.get("original_name") or out.get("original_title")
                        out["overview"] = (d.get("overview") or out.get("overview") or "")[:600]
                except Exception as e:
                    log.debug("TMDB details failed: %s", e)
        except Exception as e:
...
class Store:
    ...
    async def categories(self, type_: str) -> List[Dict]:
        ...
        if not isinstance(data, list):
            return []
        if type_ == "live":
            return [c for c in data if not is_non_islamic_religious(c.get("category_name", ""))]
        return data
    ...
    async def streams(self, type_: str, cat_id=None) -> List[Dict]:
        ...
        if not isinstance(data, list):
            return []
        if type_ == "live":
            return [it for it in data if not is_non_islamic_religious(it.get("name") or it.get("title") or "")]
        return data
    ...
    async def search(self, query: str) -> List[Tuple[str, Dict]]:
        ...
        for type_ in ("movie", "series", "live"):
            ...
            for it in items:
                if type_ == "live" and is_non_islamic_religious(it.get("name") or it.get("title") or ""):
                    continue
                iid = it.get(id_key)
                if iid is None:
                    continue
...
# ══════════════ 🌳 شجرة التصنيفات المنظمة ══════════════
HIERARCHY_CACHE: Dict[str, Dict] = {}

LIVE_NON_ISLAMIC = (
    "CHRISTIAN", "CHRIST", "CATHOLIC", "ORTHODOX", "PROTESTANT", "CHURCH", "BIBLE",
    "JEWISH", "JUDAISM", "TORAH", "SYNAGOGUE", "HINDU", "HINDUISM", "BUDDH", "BUDDHA",
    "SIKH", "SIKHISM", "SHIA", "SHIITE", "SHI'A", "SHI3A", "شيعة", "مسيحي", "مسيحية",
    "يهودي", "يهودية", "هندوسي", "هندوسية", "بوذي", "بوذية", "سيخي", "سيخية"
)

def is_non_islamic_religious(name: str) -> bool:
    s = normalize_ar(str(name or "")).upper()
    if not s:
        return False
    return any(k.upper() in s for k in LIVE_NON_ISLAMIC)

def locale_key(raw: str) -> str:
    s = str(raw or "").upper()
    rules = [
        (["EGYPT", "EGYPTE", "MASR", "مصر", " EG "], "🇪🇬 مصرية"),
        (["ARABE", "ARABIC", "ARAB", "عربي", " AR "], "🌍 عربية"),
        (["FRANCE", "FRANÇAIS", "FRANCAIS", "FRENCH", " VOD FR", "SERIE FR"], "🇫🇷 فرنسية"),
        (["TURC", "TURK", "TURKISH", "تركي", " TR "], "🇹🇷 تركية"),
        (["INDIA", "INDIAN", "HINDI", "BOLLYWOOD", " IN "], "🇮🇳 هندية"),
        (["KOREA", "KDRAMA", "KOREAN", "ASIAN", "ASIE", " KR "], "🇰🇷 كورية وآسيوية"),
        (["SPANISH", "SPAIN", "ESPAGNOL", " ES "], "🇪🇸 إسبانية"),
        (["ITALIAN", "ITALIA", "ITALY", " IT "], "🇮🇹 إيطالية"),
        (["GERMAN", "GERMANY", "DEUTSCH", " DE "], "🇩🇪 ألمانية"),
        (["PERSIAN", "IRAN", "FARSI"], "🇮🇷 إيرانية"),
        (["MAROC", "MOROCCO", "ALGERI", "ALGERIA", "TUNIS", "TUNISIA", "MAGHREB"], "🌍 مغاربية"),
    ]
    for keys,label in rules:
        if any(k in s for k in keys): return label
    return "🌍 أجنبية ومتنوعة"

def genre_key(raw: str, type_: str = "movie") -> str:
    s = str(raw or "").upper()
    if is_adult_name(raw): return "🔞 للكبار فقط"
    if type_ == "live":
        for keys, icon, label in LIVE_GENRE_RULES:
            if any(k in s for k in keys): return f"{icon} {label}"
        return "📺 منوعات"
    for keys, icon, label in GENRE_RULES:
        if any(k in s for k in keys): return f"{icon} {label}"
    return "🎬 عام"

def build_media_tree(cats: List[Dict], type_: str) -> Dict[str, Dict]:
    """للأفلام/المسلسلات: بلد/لغة ← نوع ← الأقسام الخام. للقنوات: نوع ← باقة."""
    tree: Dict[str, Dict] = {}
    for c in cats:
        raw = str(c.get("category_name") or "").strip()
        if not raw: continue
        if type_ == "live":
            if is_non_islamic_religious(raw): continue
            top = genre_key(raw, "live")
            tree.setdefault(top, {"count":0,"categories":[]})
            tree[top]["count"] += int(c.get("stream_count") or 0)
            tree[top]["categories"].append(c)
        else:
            top = locale_key(raw)
            sub = genre_key(raw, type_)
            tree.setdefault(top, {"count":0,"genres":{}})
            tree[top]["count"] += int(c.get("stream_count") or 0)
            tree[top]["genres"].setdefault(sub, {"count":0,"categories":[]})
            tree[top]["genres"][sub]["count"] += int(c.get("stream_count") or 0)
            tree[top]["genres"][sub]["categories"].append(c)
    return tree

def quality_from_name(name: str) -> str:
    m = re.search(r"(?i)(4K|8K|UHD|FHD|1080P|1080|720P|720|576P|576|480P|480|HD|SD)", str(name or ""))
    return m.group(1).upper() if m else ""

def base_channel_name(name: str) -> str:
    s = clean_name(name)
    s = re.sub(r"(?i)\b(4K|8K|UHD|FHD|1080P?|720P?|576P?|480P?|HD|SD)\b", "", s)
...
def categories_kb(cats: List[Dict], type_: str) -> InlineKeyboardMarkup:
    """واجهة احتياطية للأقسام الخام، مع صفوف نظيفة."""
    btns=[]
    for cat in cats:
        cid=cat.get("category_id"); name=clean_name(cat.get("category_name")); cnt=cat.get("stream_count")
        label=f"{name} ({cnt})" if cnt else name
        btns.append(InlineKeyboardButton(label[:32], callback_data=f"c:{type_}:{cid}:0"))
    rows=grid(btns,2); rows.append(back_main_row()); return InlineKeyboardMarkup(rows)

def hierarchy_top_kb(tree: Dict, type_: str) -> InlineKeyboardMarkup:
    btns=[]
    for key,data in sorted(tree.items(), key=lambda kv: (-kv[1].get("count",0), kv[0])):
        btns.append(InlineKeyboardButton(f"{key} ({data.get('count',0):,})"[:34], callback_data=f"h:{type_}:{list(tree.keys()).index(key)}"))
    rows=grid(btns,2); rows.append(back_main_row()); return InlineKeyboardMarkup(rows)

def hierarchy_second_kb(type_: str, top_idx: int, tree: Dict) -> InlineKeyboardMarkup:
    tops=list(tree.items())
    if not (0 <= top_idx < len(tops)): return hierarchy_top_kb(tree,type_)
    top_name, data=tops[top_idx]
    btns=[]
    if type_ == "live":
        for i,cat in enumerate(sorted(data["categories"], key=lambda c: clean_name(c.get("category_name")))):
            label=clean_name(cat.get("category_name")); cnt=int(cat.get("stream_count") or 0)
            btns.append(InlineKeyboardButton(f"📦 {label[:25]} ({cnt})", callback_data=f"p:live:{top_idx}:{i}:0"))
    else:
        genres=data.get("genres",{})
        for i,(gname,gdata) in enumerate(sorted(genres.items(), key=lambda kv: (-kv[1]["count"],kv[0]))):
            btns.append(InlineKeyboardButton(f"{gname} ({gdata['count']:,})"[:34], callback_data=f"hg:{type_}:{top_idx}:{i}"))
    rows=grid(btns,2)
    rows.append([InlineKeyboardButton("🔙 التصنيفات الرئيسية", callback_data=f"t:{type_}")])
    rows.append(back_main_row())
    return InlineKeyboardMarkup(rows)

def hierarchy_third_kb(type_: str, top_idx: int, genre_idx: int, tree: Dict) -> InlineKeyboardMarkup:
    tops=list(tree.items()); top_name,data=tops[top_idx]
    genres=list(sorted(data["genres"].items(), key=lambda kv: (-kv[1]["count"],kv[0])))
    gname,gdata=genres[genre_idx]
    btns=[]
    for cat in sorted(gdata["categories"], key=lambda c: clean_name(c.get("category_name"))):
        cid=cat.get("category_id"); label=clean_name(cat.get("category_name")); cnt=int(cat.get("stream_count") or 0)
        btns.append(InlineKeyboardButton(f"📂 {label[:24]} ({cnt})", callback_data=f"c:raw:{type_}:{cid}:0"))
    rows=grid(btns,2)
    rows.append([InlineKeyboardButton("🔙 الأنواع", callback_data=f"h:{type_}:{top_idx}")])
    rows.append([InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="main")])
    return InlineKeyboardMarkup(rows)

def live_packages_kb(top_idx: int, tree: Dict) -> InlineKeyboardMarkup:
    tops=list(tree.items()); top_name,data=tops[top_idx]
    btns=[]
    for i,cat in enumerate(sorted(data["categories"], key=lambda c: clean_name(c.get("category_name")))):
        label=clean_name(cat.get("category_name")); cnt=int(cat.get("stream_count") or 0)
        btns.append(InlineKeyboardButton(f"📦 {label[:27]} ({cnt})", callback_data=f"p:live:{top_idx}:{i}:0"))
    rows=grid(btns,2); rows.append([InlineKeyboardButton("🔙 باقات القنوات", callback_data="t:live")]); rows.append(back_main_row()); return InlineKeyboardMarkup(rows)


def display_item_name(item: Dict) -> str:
    ar = clean_name(item.get("name_ar") or item.get("title_ar") or item.get("name") or item.get("title"))
    en = clean_name(item.get("name_en") or item.get("title_en") or item.get("original_name") or item.get("original_title"))
    if en and normalize_ar(en) != normalize_ar(ar):
        return f"{ar} | {en}"
    return ar
...
def items_kb(items: List[Dict], type_: str, cat_id: str, page: int) -> InlineKeyboardMarkup:
    ...
    for it in slice_:
        iid = it.get(id_key)
        name = display_item_name(it)
        btns.append(InlineKeyboardButton(f"{ITEM_ICON[type_]} {name[:30]}",
...
def item_detail_kb(uid: int, db: DB, type_: str, item: Dict, cat_id: str, page: int, xt: Xtream,
                   title: str = "", poster: str = "", live_variants: Optional[List[Dict]] = None) -> InlineKeyboardMarkup:
    """أزرار كارت العمل: مشاهدة + بث (بدون مشغل خارجي) + مفضلة + رجوع — متناسقة."""
    rows = []
    id_key = "series_id" if type_ == "series" else "stream_id"
    iid = item.get(id_key)
    if type_ == "live":
        variants = live_variants or []
        if len(variants) > 1:
            for v in variants:
                vid = v.get("stream_id")
                q = quality_from_name(v.get("name") or "") or "جودة متاحة"
                rows.append([InlineKeyboardButton(f"▶️ {q}", url=xt.live_url(vid))])
        else:
            rows.append([InlineKeyboardButton("▶️ مشاهدة مباشرة", url=xt.live_url(iid))])
    elif type_ == "movie":
        ext = item.get("container_extension") or "mp4"
        rows.append([InlineKeyboardButton("▶️ مشاهدة مباشرة", url=xt.movie_url(iid, ext))])
        rows.append([InlineKeyboardButton(f"🔗 الرابط الأصلي ({ext})", url=xt.movie_url(iid, ext))])
    elif type_ == "series":
...
def episode_play_kb(ep: Dict, series_id, season: int, page: int, xt: Xtream,
    ...
    rows = [[InlineKeyboardButton("▶️ مشاهدة الحلقة 🎬",
                                  url=xt.episode_url(eid, ep.get("container_extension") or "mp4"))]]
...
class CinemaBot:
    ...
    async def _stats_text(self, msg):
        ...
        lines.append("🔗 التشغيل: روابط المصدر الأصلية المباشرة")
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if type_ == "live":
            cats = [c for c in cats if not is_non_islamic_religious(c.get("category_name", ""))]
        if not cats:
            await self._safe_edit(q, f"❌ لا توجد أقسام في {TYPE_LABEL[type_]} حالياً.", InlineKeyboardMarkup([back_main_row()]))
            return
        tree=build_media_tree(cats,type_)
        HIERARCHY_CACHE[type_]=tree
        if type_ == "live":
            text=f"📡✨ <b>القنوات المباشرة</b> ✨\\n━━━━━━━━━━━━━━━\\nاختر نوع القنوات أولاً، ثم الباقة، ثم القناة:"
        else:
            text=(f"{TYPE_LABEL[type_]} ✨\\n━━━━━━━━━━━━━━━\\n"
                  f"اختر البلد/اللغة أولاً، ثم النوع، ثم القسم:")
        await self._safe_edit(q,text,hierarchy_top_kb(tree,type_))
    ...
    async def _require_adult(self, q, uid: int) -> bool:
        ...
        await self._safe_edit(
            q,
            "🔞✨ <b>هذا القسم مخصص لمالك البوت فقط</b> ✨\\n━━━━━━━━━━━━━━━\\n"
    ...
    async def _show_item(self, q, uid: int, type_: str, iid: str, cat_id: str, page: int):
        ...
        display_ar = meta.get("title_ar") or meta.get("title") or name
        display_en = meta.get("original_title") or ""
        title_line = f"{esc(display_ar)}"
        if display_en and normalize_ar(display_en) != normalize_ar(display_ar):
            title_line += f"\n🇬🇧 <b>{esc(display_en)}</b>"
        lines = [f"{ITEM_ICON[type_]}✨ <b>{title_line}</b> ✨", "━━━━━━━━━━━━━━━"]
        if year:
        ...
        if runtime:
            try:
                mins=int(runtime); lines.append(f"⏱️ المدة: <b>{mins//60}س {mins%60:02d}د</b>")
            except Exception:
                pass
        if overview:
            lines.append(f"\n📝 <i>{esc(str(overview)[:420])}</i>")
        text = "\n".join(lines)
        ...
        if type_ == "live":
            ...
            try:
                candidates = await self.store.streams("live", cat) if cat is not None else []
                target_base = base_channel_name(name)
                live_variants = [v for v in candidates if base_channel_name(v.get("name") or "") == target_base]
                live_variants.sort(key=lambda v: (quality_from_name(v.get("name") or "") == "", quality_from_name(v.get("name") or "")))
            except Exception:
                live_variants = None
        kb = item_detail_kb(uid, self.db, type_, item, cat_id, page, self.xt, name, poster, live_variants)
    ...
    async def _play_episode(self, q, episode_id: str, series_id: str, season: int, page: int):
        ...
        lines.append("\n🔗 <i>الرابط الأصلي المباشر من المصدر</i>")
    ...
    async def on_callback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "main":
                ...
                _, t, idx = data.split(":")
                tree=HIERARCHY_CACHE.get(t) or build_media_tree(await self.store.categories(t), t)
                HIERARCHY_CACHE[t]=tree
                await self._safe_edit(q, f"✨ <b>{esc(TYPE_LABEL[t])}</b> ✨\n━━━━━━━━━━━━━━━\nاختر {'الباقة' if t=='live' else 'النوع'}:", hierarchy_second_kb(t,int(idx),tree))
            elif data.startswith("hg:"):
                _, t, top_idx, genre_idx = data.split(":")
                tree=HIERARCHY_CACHE.get(t)
                if not tree:
                    tree=build_media_tree(await self.store.categories(t),t); HIERARCHY_CACHE[t]=tree
                await self._safe_edit(q, "📂 <b>الأقسام</b>\n━━━━━━━━━━━━━━━\nاختر القسم:", hierarchy_third_kb(t,int(top_idx),int(genre_idx),tree))
            elif data.startswith("p:live:"):
                _, t, top_idx, pkg_idx, page = data.split(":")
                tree=HIERARCHY_CACHE.get("live")
                if not tree:
                    tree=build_media_tree(await self.store.categories("live"),"live"); HIERARCHY_CACHE["live"]=tree
                tops=list(tree.items()); data_top=tops[int(top_idx)][1]
                cats_sorted=sorted(data_top["categories"], key=lambda c: clean_name(c.get("category_name")))
                cat=cats_sorted[int(pkg_idx)]
                await self._show_items(q,"live",str(cat.get("category_id")),int(page))
            elif data.startswith("c:raw:"):
                _, _, t, cid, page = data.split(":")
                ...
                await self._safe_edit(
                    ...
                    "🔗 <b>رابط مباشر:</b> أرسل أي رابط HTTP/HTTPS وسيتم استخدامه كما هو بدون مشغل خارجي.\n"
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if url_match:
            ...
            if raw_url.startswith(("http://", "https://")):
                link = raw_url
                await update.message.reply_text(
                    "🎬✨ <b>الرابط الأصلي جاهز</b> ✨\n"
                    "━━━━━━━━━━━━━━━\n"
                    "🔗 تم الحفاظ على رابط المصدر بدون أي مشغل خارجي أو بارامترات إضافية.\n\n"
    ...
    def run(self):
        ...
        log.info("🚀 %s يعمل الآن — السيرفر: %s | التشغيل: روابط المصدر الأصلية", BOT_NAME, self.xt.base)
