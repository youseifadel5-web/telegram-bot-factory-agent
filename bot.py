...
async def _show_hub_external(q, uid):
    """Render the external hub with its private admin button."""
    if not uid:
        return False
...
async def _show_external_admin(q, uid):
    ...
    rows.append([InlineKeyboardButton("🔙 بوت سينماء", callback_data="admin:back")])
...
async def handle_addbot_callback(upd, uid, data):
    ...
    if data == "addbot:back":
        await _show_hub_external(q, uid)
        return True
    ...
    while True:
        try:
            ...
            for raw in raw_updates:
                ...
                try:
                    upd = TeleUpdate.de_json(json.dumps(raw, ensure_ascii=False))
                    if upd is None:
                        continue
                    ...
                    if is_callback(upd):
                        ...
                        if data.startswith("admin:") or data.startswith("addbot:") or data == "unified_search":
                            # These controls are owned by the external PTB host.
                            # Build a real python-telegram-bot Update from the raw
                            # Telegram payload; never pass a pyTelegramBotAPI
                            # CallbackQuery to PTB handlers.
                            ptb_update = youseif.Update.de_json(raw, PTB_BOT)
                            if ptb_update is None:
                                continue
                            if data.startswith("admin:"):
                                q = ptb_update.callback_query
                            ...
                            if data.startswith("addbot:"):
                                await handle_addbot_callback(ptb_update, uid, data)
                                continue
                            ...
                            q = ptb_update.callback_query
                            await q.answer()
                            if uid and q.message:
                                await PTB_BOT.send_message(
                                    chat_id=q.message.chat.id,
                                    text="🔍 <b>بحث</b>\n\nاكتب اسم الفيلم أو المسلسل أو القناة للبحث في كل البوتات:",
                                    parse_mode=youseif.ParseMode.HTML,
                                )
                            continue
                        ...
                        if data == "hub_home":
                            ptb_update = youseif.Update.de_json(raw, PTB_BOT)
                            if ptb_update is not None and ptb_update.callback_query:
                                await ptb_update.callback_query.answer()
                                await _show_hub_external(ptb_update.callback_query, uid)
                            USER_MODE[uid] = "cinema"
                            continue
