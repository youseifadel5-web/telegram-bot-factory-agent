...
class Store:
    ...
    async def search(self, query: str) -> List[Tuple[str, Dict]]:
        """بحث ذكي عربي/إنجليزي — توحيد الحروف العربية قبل المطابقة."""
        ...
        for type_ in ("movie", "series", "live"):
            ...
            for it in items:
                ...
                if q in name and it.get(id_key) is not None:
...
def item_detail_kb(uid: int, db: DB, type_: str, item: Dict, cat_id: str, page: int, xt: Xtream,
    ...
    iid = item.get(id_key)
    if type_ == "live":
        rows.append([InlineKeyboardButton("▶️ مشاهدة البث المباشر 🎬",
                                          url=player_link(xt.live_url(iid), title))])
    elif type_ == "movie":
        rows.append([InlineKeyboardButton("▶️ مشاهدة الفيلم 🎬",
