...
def clean_url(u: str) -> str:
    """يرجع الرابط خام (URL-Decode) — يشتغل في VLC/MX Player/المتصفح مباشرة."""
    s = str(u or "").strip()
    # فك التشفير المتكرر (ممكن يكون متشفر مرتين)
    for _ in range(3):
        if "%3A" in s or "%2F" in s:
            s = _unquote(s)
        else:
            break
    return s
...
def player_link(video_url: str, title: str = "", sub: str = "") -> str:
    ...
    """
    video_url = clean_url(video_url)   # فك التشفير قبل الاستخدام
    sub = clean_url(sub) if sub else ""
...
class Store:
    ...
    def _similar(self, a: str, b: str) -> bool:
        """تشابه متسامح مع الأخطاء الإملائية (jon wik ≈ john wick)."""
        import difflib
        if a in b or b in a:
            return True
        # مقارنة بالكلمات
        for w in a.split():
            for w2 in b.split():
                if len(w) > 2 and len(w2) > 2 and difflib.SequenceMatcher(None, w, w2).ratio() >= 0.78:
                    return True
        return difflib.SequenceMatcher(None, a, b).ratio() >= 0.72

    async def search(self, query: str) -> List[Tuple[str, Dict]]:
        """بحث ذكي عربي/إنجليزي + تسامح إملائي + الأسماء البديلة (ترولز ↔ Trolls)."""
        q = normalize_ar(query)
        if not q:
            return []
        ...
        for type_ in ("movie", "series", "live"):
            ...
            for it in items:
                ...
                # مطابقة بالاسم الأصلي أو المتسامح إملائياً أو بالاسم البديل المحفوظ
                if q in name or self._similar(q, name) or (type_, str(iid)) in alias_ids:
...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔥 الأكثر مشاهدة", callback_data="sm:top"),
         InlineKeyboardButton("⭐ الأعلى تقييماً", callback_data="sm:rated")],
        [InlineKeyboardButton("🆕 المضاف حديثاً", callback_data="sm:new"),
         InlineKeyboardButton("🕒 آخر مشاهدة", callback_data="sm:recent")],
    ...
    async def on_callback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "main":
                ...
                # ⭐ الأقسام الذكية: الأكثر مشاهدة / الأعلى تقييماً / المضاف حديثاً / آخر مشاهدة
                mode = data.split(":", 1)[1]
                await self._safe_edit(q, "⏳ جاري التحضير...")
                type_ = "movie"
                items = await self.store.streams(type_, None)
                id_key = "stream_id"
                if mode == "rated":
                    items = sorted(items, key=lambda x: float(x.get("rating") or 0), reverse=True)[:40]
                    title = "⭐ الأعلى تقييماً"
                elif mode == "new":
                    items = sorted(items, key=lambda x: int(x.get("added") or 0), reverse=True)[:40]
                    title = "🆕 المضاف حديثاً"
                elif mode == "top":
                    # الأكثر مشاهدة = الأعلى تقييماً × شعبية (أو حسب سجل المشاهدات)
                    with self.db._con() as c:
                        top_ids = [r[0] for r in c.execute(
                            "SELECT item_id, COUNT(*) AS n FROM views GROUP BY item_id ORDER BY n DESC LIMIT 40").fetchall()]
                    idset = set(top_ids)
                    items = [it for it in items if str(it.get(id_key)) in idset] or                             sorted(items, key=lambda x: float(x.get("rating") or 0), reverse=True)[:40]
                    title = "🔥 الأكثر مشاهدة"
                else:  # recent — آخر ما شاهده المستخدم الحالي
                    with self.db._con() as c:
                        rec = [r[0] for r in c.execute(
                            "SELECT item_id FROM views WHERE user_id=? ORDER BY id DESC LIMIT 40",
                            (q.from_user.id,)).fetchall()]
                    idset = set(rec)
                    items = [it for it in items if str(it.get(id_key)) in idset]
                    title = "🕒 آخر مشاهدة"
                if not items:
                    await self._safe_edit(q, f"❌ لا توجد عناصر في «{title}» بعد.", main_menu_kb())
                else:
                    btns = [InlineKeyboardButton(f"🎞️ {clean_name(it.get('name'))[:26]}",
                                                 callback_data=f"i:movie:{it.get(id_key)}:all:0")
                            for it in items[:ITEMS_PER_PAGE]]
                    rows = grid(btns, 2)
                    rows.append(back_main_row())
                    await self._safe_edit(
                        q,
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if url_match:
            ...
            if re.search(r"\.(m3u8|mp4|mkv|webm|mov|ts)(\?|$)", raw_url, re.I):
                clean_raw = clean_url(raw_url)          # فك التشفير — يشتغل في VLC
                link = player_link(clean_raw, "رابط مباشر")
                await update.message.reply_text(
                    "🎬✨ <b>تم تجهيز الرابط</b> ✨\n"
                    "━━━━━━━━━━━━━━━\n"
                    f"🔐 <i>رابط المشغل صالح {LINK_TTL // 3600} ساعة</i>\n"
                    f"📡 <i>الرابط الخام يعمل في VLC وMX Player مباشرة</i>",
                    parse_mode=ParseMode.HTML,
