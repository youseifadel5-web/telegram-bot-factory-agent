...
async def build_content_hierarchy(store, type_: str) -> List[Dict]:
    ...
    if not cats:
        ...
        return []
    # بعض Xtream servers لا يعيد stream_count داخل categories؛ نحسب العدد من العناصر نفسها.
    counts = {}
    try:
    ...
    # قسم محمي ظاهر باسم عام فقط؛ لا نكشف اسم/نوع محتوى الكبار للمستخدم.
    # يتم الوصول الفعلي إليه فقط عبر صلاحية المالك.
    protected_ids = []
    for c in cats:
        raw_name = str(c.get("category_name") or "")
        if is_adult_name(raw_name):
            protected_ids.append(str(c.get("category_id")))
    if protected_ids:
        groups["تصنيف خاص"] = {
            "icon": "🔐",
            "countries": {"🔒 محتوى محمي": {"ids": protected_ids, "count": 0}}
        }
...
class CinemaBot:
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if type_ == "live":
            ...
            rows = grid([InlineKeyboardButton(
                f"{g['icon']} {'🔞 للكبار فقط' if g.get('key') == 'تصنيف خاص' and is_admin(q.from_user.id) else g['key']}",
    ...
    async def _show_genre_countries(self, q, type_: str, idx: int):
        ...
        if g.get("key") == "تصنيف خاص" and not is_admin(q.from_user.id):
            await self._require_adult(q, q.from_user.id)
            return
        if g.get("key") == "تصنيف خاص":
            entries = [("🔞 محتوى خاص", d) for _, d in entries]
    ...
    async def _show_genre_items(self,q,type_:str,gidx:int,cidx:int,page:int=0):
        ...
        for rid in d["ids"]:
            for it in await self.store.streams(type_,rid):
                iid=it.get(id_key)
                allowed = is_admin(q.from_user.id) or not is_adult_name(it.get("name") or "")
                if iid is not None and iid not in seen and allowed:
    ...
    async def _require_adult(self, q, uid: int) -> bool:
        """حماية صارمة: مالك البوت فقط يمكنه الوصول. لا يتم كشف أسماء المحتوى للمستخدمين."""
        if is_admin(uid):
            return True
    ...
    async def _find_item(self, type_: str, iid: str, cat_id: str = "") -> Optional[Dict]:
        ...
        # lp:genre_index:package_index للقنوات
        ...
        # fallback: محاولة البحث في all-items كما في السلوك الأصلي.
        try:
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # 🔐 المستخدمون لا يحصلون على باسورد المحتوى المقيد؛ الوصول للمالك فقط.
