...
def groups_kb(groups: Dict[str, Dict], type_: str) -> InlineKeyboardMarkup:
    """لوحة الفئات المدمجة — شبكة عمودين مع إجمالي الأعمال لكل فئة.
    نرسل رقم الفئة فقط في الـ callback (قصير، لا يتجاوز حد 64 بايت)."""
    btns = []
    for idx, (gname, gdata) in enumerate(groups.items()):
        btns.append(InlineKeyboardButton(f"{gname} ({gdata['count']:,})"[:34],
                                         callback_data=f"g:{type_}:{idx}"))
    rows = grid(btns, 2)
    rows.append(back_main_row())
    return InlineKeyboardMarkup(rows)
...
class CinemaBot:
    ...
    async def _show_items(self, q, type_: str, cat_id: str, page: int):
        ...
        if cat_id.startswith("g") and cat_id[1:].isdigit():
            ...
            idx = int(cat_id[1:])
            entries = GROUPS_CACHE.get(type_) or []
            if idx >= len(entries):
    ...
    async def on_callback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "main":
                ...
                # 🌳 فئة مدمجة → عرض كل عناصرها مجمعة من الأقسام الخام تحتها
                _, t, idx = data.split(":", 2)
                entries = GROUPS_CACHE.get(t)
                if not entries or not idx.isdigit() or int(idx) >= len(entries):
                    # الكاش انتهى — أعد بناء الفئات
                    await self._show_categories(q, t)
                else:
                    gname, gdata = entries[int(idx)]
