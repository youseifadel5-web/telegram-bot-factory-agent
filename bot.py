...
def clean_name(raw) -> str:
    """يشيل البادئات الفنية [AR]_ / [FR]_ ويظبط الاسم."""
    s = str(raw or "").strip()
    s = re.sub(r"^\[[A-Za-z]{1,4}\][_\-\s]*", "", s)
    return s.strip(" _-") or str(raw or "بدون عنوان")
...
# ══════════════ 🌳 الأقسام الشجرية (مجموعات → أقسام فرعية) ══════════════
CATEGORY_GROUPS = [
    {
        "match": ["EG", "ARAB", "AR ", "ARABIC", "MASr", "مصر"],
        "flag": "🇪🇬", "name": "المحتوى المصري والعربي", "icon": "🎬",
    },
    {
        "match": ["TR", "TURK"],
        "flag": "🇹🇷", "name": "المحتوى التركي", "icon": "🎭",
    },
    {
        "match": ["FR", "VF", "FRENCH"],
        "flag": "🇫🇷", "name": "المحتوى الفرنسي", "icon": "🎬",
    },
    {
        "match": ["EN", "US", "UK", "ENG", "VOSTFR"],
        "flag": "🇬🇧", "name": "المحتوى الأجنبي", "icon": "🎬",
    },
    {
        "match": ["DE", "GER"],
        "flag": "🇩🇪", "name": "المحتوى الألماني", "icon": "🎬",
    },
    {
        "match": ["ES", "SPAN"],
        "flag": "🇪🇸", "name": "المحتوى الإسباني", "icon": "🎬",
    },
    {
        "match": ["TR", "TUR"],
        "flag": "🇹🇷", "name": "المحتوى التركي", "icon": "🎭",
    ...
    {
        "match": ["SPORT", "BEIN", "SSC"],
        "flag": "⚽", "name": "الرياضة", "icon": "⚽",
    },
    {
        "match": ["KIDS", "CHILD", "CARTOON", "ANIM"],
        "flag": "🧸", "name": "الأطفال والأنيميشن", "icon": "🧸",
    },
    {
        "match": ["NEWS", "INFO"],
        "flag": "📰", "name": "الأخبار", "icon": "📰",
    },
    {
        "match": ["DOC", "DOCUMENTARY"],
        "flag": "📚", "name": "الوثائقيات", "icon": "📚",
    },
    {
        "match": ["ISLAMIC", "RELIGIOUS", "QURAN"],
        "flag": "🕌", "name": "المحتوى الديني", "icon": "🕌",
    },
]


def cat_group_key(cat_name: str) -> str:
    """يحدد المجموعة الرئيسية لقسم من اسمه الخام."""
    upper = re.sub(r"[^\w\s&/-]", " ", str(cat_name or "").upper())
    tokens = set(re.split(r"[\s&/|_\-]+", upper))
    for g in CATEGORY_GROUPS:
        if tokens & set(g["match"]):
            return g["name"]
    return "🌍 أقسام أخرى"


def group_categories(cats: List[Dict]) -> Dict[str, List[Dict]]:
    """يجمع الأقسام في مجموعات شجرية مرتبة."""
    groups: Dict[str, List[Dict]] = {}
    for c in cats:
        key = cat_group_key(c.get("category_name") or "")
...
def categories_kb(cats: List[Dict], type_: str) -> InlineKeyboardMarkup:
    ...
    for cat in cats:
        ...
        name = pretty_category(cat.get("category_name"))
...
def groups_kb(groups: Dict[str, List[Dict]], type_: str) -> InlineKeyboardMarkup:
    """لوحة المجموعات الرئيسية — شبكة عمودين مع إجمالي الأعمال."""
    btns = []
    for gname, gcats in groups.items():
        total = sum(c.get("stream_count") or 0 for c in gcats)
        ginfo = next((g for g in CATEGORY_GROUPS if g["name"] == gname), None)
        flag = ginfo["flag"] if ginfo else "🌍"
        label = f"{flag} {gname.replace('🌍 ', '')} ({total:,})"
        btns.append(InlineKeyboardButton(label[:30], callback_data=f"g:{type_}:{gname[:18]}"))
    rows = grid(btns, 2)
    rows.append(back_main_row())
    return InlineKeyboardMarkup(rows)
...
class CinemaBot:
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if not cats:
            ...
            return
        # 🌳 شجرة الأقسام: مجموعات رئيسية → أقسام فرعية
        groups = group_categories(cats)
        if len(groups) > 1:
            await self._safe_edit(
                q,
                f"✨ <b>{TYPE_LABEL[type_]}</b> ✨\n━━━━━━━━━━━━━━━\n"
                f"🌳 <b>{len(groups)} مجموعة</b> / 📂 {len(cats)} قسم — اختر المجموعة:",
                groups_kb(groups, type_))
        else:
            await self._safe_edit(
                q,
                f"✨ <b>{TYPE_LABEL[type_]}</b> ✨\n━━━━━━━━━━━━━━━\n"
    ...
    async def on_callback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "main":
                ...
                # 🌳 مجموعة رئيسية → عرض أقسامها الفرعية فقط
                _, t, gname = data.split(":", 2)
                cats = await self.store.categories(t)
                groups = group_categories(cats)
                gcats = groups.get(gname) or next(
                    (v for k, v in groups.items() if k.startswith(gname)), [])
                if not gcats:
                    await self._safe_edit(q, "❌ المجموعة غير موجودة.", main_menu_kb())
                else:
                    total = sum(c.get("stream_count") or 0 for c in gcats)
                    rows = grid([
                        InlineKeyboardButton(
                            f"{pretty_category(c.get('category_name'))} ({c.get('stream_count') or 0:,})"[:30],
                            callback_data=f"c:{t}:{c.get('category_id')}:0")
                        for c in gcats
                    ], 2)
                    rows.append([InlineKeyboardButton(f"🔙 {TYPE_LABEL[t]}", callback_data=f"t:{t}")])
                    rows.append(back_main_row())
                    await self._safe_edit(
                        q,
                        f"🌳✨ <b>{esc(gname)}</b> ✨\n━━━━━━━━━━━━━━━\n"
                        f"📂 <b>{len(gcats)} قسم</b> — 🎞️ {total:,} عمل:",
                        InlineKeyboardMarkup(rows))
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if url_match:
            ...
            if re.search(r"\.(m3u8|mp4|mkv|webm|mov|ts)(\?|$)", raw_url, re.I):
                link = player_link(raw_url, "رابط مباشر")
                await update.message.reply_text(
                    "🎬✨ <b>تم تحويل الرابط لمشغّلك</b> ✨\n"
                    "━━━━━━━━━━━━━━━\n"
                    f"🔐 <i>رابط مشفّر صالح {LINK_TTL // 3600} ساعة</i>",
                    parse_mode=ParseMode.HTML,
