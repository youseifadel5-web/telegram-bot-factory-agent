...
• تشغيل ويب مباشر عبر رابط Worker — أزرار مشاهدة منظمة
...
# ══════════════ أسرار من GitHub Secrets / Environment ══════════════
...
# مالك البوت فقط — لا يوجد وضع "كل المستخدمين أدمن".
...
# رابط تشغيل الويب الرسمي — لا يُستخدم مع أي تطبيق مشغل خارجي.
...
# تصنيفات المستوى الأول للأفلام والمسلسلات: اللغة/المنشأ أولاً، ثم التفاصيل داخل القائمة.
TOP_LOCALE_RULES = [
    (["EGYPT", "EGYPTE", "MASR", "MASR", "مصر", "EG"], "🇪🇬", "أفلام/مسلسلات مصرية"),
    (["ARABE", "ARABIC", "ARAB", "عربي", "AR"], "🇸🇦", "أفلام/مسلسلات عربية"),
    (["FRANCE", "FRANÇAIS", "FRANCAIS", "FRENCH", "VOD FR", "SERIE FR", "FR"], "🇫🇷", "أفلام/مسلسلات فرنسية"),
    (["TURC", "TURK", "TURKISH", "تركي", "TR"], "🇹🇷", "أفلام/مسلسلات تركية"),
    (["INDIA", "INDIAN", "HINDI", "BOLLYWOOD", "IN"], "🇮🇳", "أفلام/مسلسلات هندية"),
    (["KOREA", "KDRAMA", "KOREAN", "ASIAN", "ASIE", "KR"], "🇰🇷", "أفلام/مسلسلات كورية وآسيوية"),
    (["SPANISH", "SPAIN", "ESPAGNOL", "ES"], "🇪🇸", "أفلام/مسلسلات إسبانية"),
    (["ITALIAN", "ITALIA", "ITALY", "IT"], "🇮🇹", "أفلام/مسلسلات إيطالية"),
    (["GERMAN", "GERMANY", "DEUTSCH", "DE"], "🇩🇪", "أفلام/مسلسلات ألمانية"),
    (["PERSIAN", "IRAN", "FARSI"], "🇮🇷", "أفلام/مسلسلات إيرانية"),
    (["MOROCCO", "MAROC", "MAGHREB", "ALGERIA", "ALGERI", "TUNISIA", "TUNIS"], "🌍", "أفلام/مسلسلات مغاربية"),
]

def is_non_islamic_religious(name: str) -> bool:
    """يحجب القنوات الدينية غير الإسلامية من واجهة المستخدم."""
    s = normalize_ar(str(name or "")).upper()
    if not s:
        return False
    islamic = ("ISLAM", "ISLAMIC", "QURAN", "CORAN", "MUSLIM", "MOSQUE", "MASJID", "SUNNI", "سني", "اسلام", "إسلام", "قران", "قرآن")
    if any(k in s for k in islamic):
        return False
    non_islamic = (
        "CHRISTIAN", "CHRIST", "CATHOLIC", "ORTHODOX", "PROTESTANT", "CHURCH", "BIBLE",
        "JEWISH", "JUDAISM", "TORAH", "SYNAGOGUE", "HINDU", "HINDUISM", "BUDDH",
        "BUDDHA", "SIKH", "SIKHISM", "SHIA", "SHIITE", "SHI'A", "SHI3A", "شيعة", "مسيحي",
        "مسيحية", "يهودي", "يهودية", "هندوسي", "بوذي", "سيخي"
    )
    if any(k in s for k in non_islamic):
        return True
    # فئة دينية عامة بلا علامة إسلامية: لا نعرضها للمستخدمين.
    religious_generic = ("RELIGION", "RELIGIOUS", "RELIGIEUX", "RELIGIEUSE", "RELIGIO", "ديني", "دينية")
    return any(k in s for k in religious_generic)

def _top_locale_category(raw: str, type_: str) -> str:
    s = str(raw or "").strip()
    if not s:
        return "🌐 أجنبي / متنوع" if type_ != "live" else "📡 باقة عامة"
    if type_ == "live":
        return f"📡 {clean_name(s)[:40]}"
    if is_adult_name(s):
        return "🔞 محتوى للكبار فقط"
    upper = " " + re.sub(r"[^\w\s&/-]", " ", s.upper()) + " "
    prefix = "أفلام" if type_ == "movie" else "مسلسلات"
    for keywords, icon, label in TOP_LOCALE_RULES:
        for kw in keywords:
            if re.search(rf"(?<![A-Z]){re.escape(kw.upper())}(?![A-Z])", upper):
                label = label.replace("أفلام/مسلسلات", prefix)
                return f"{icon} {label}"
    # أي محتوى غير مصنف لغوياً يذهب إلى أجنبي/متنوع بدلاً من خلطه مع الفئات العربية.
    return f"🌍 {prefix} أجنبية"

def cat_group_key(cat_name: str, type_: str = "movie") -> str:
    return _top_locale_category(cat_name, type_)

def group_categories(cats: List[Dict], type_: str = "movie") -> Dict[str, Dict]:
    """ينشئ فئات رئيسية واضحة؛ القنوات تبقى كل باقة فيها مستقلة."""
    groups: Dict[str, Dict] = {}
    for c in cats:
        raw = str(c.get("category_name") or "").strip()
        if type_ == "live" and is_non_islamic_religious(raw):
            continue
        key = cat_group_key(raw, type_)
        cnt = int(c.get("stream_count") or 0)
        if key in groups:
            groups[key]["ids"].append(str(c.get("category_id")))
            groups[key]["count"] += cnt
        else:
            groups[key] = {"ids": [str(c.get("category_id"))], "count": cnt}
    return dict(sorted(groups.items(), key=lambda kv: (kv[0] == "🌍 أفلام/مسلسلات أجنبية", kv[0])))
...
def player_link(video_url: str, title: str = "") -> str:
    """يبني رابط Worker المباشر بصيغة url + title + exp + sig فقط."""
...
class Xtream:
    ...
    # ── روابط المصدر الخام (تُوقّع لاحقاً داخل رابط الويب) ──
...
class Store:
    ...
    async def streams(self, type_: str, cat_id=None) -> List[Dict]:
        ...
        if not isinstance(data, list):
            return []
        if type_ == "live":
            allowed_ids = {str(c.get("category_id")) for c in await self.categories("live")}
            return [
                it for it in data
                if (it.get("category_id") is None or str(it.get("category_id")) in allowed_ids)
                and not is_non_islamic_religious(it.get("name") or it.get("title") or "")
            ]
        return data
    ...
    async def search(self, query: str, user_id: int = 0) -> List[Tuple[str, Dict]]:
        ...
        for type_ in ("movie", "series", "live"):
            ...
            for it in items:
                if type_ == "live" and is_non_islamic_religious(it.get("name") or it.get("title") or ""):
                    continue
                if not is_admin(user_id) and is_adult_name(it.get("name") or it.get("title") or ""):
                    continue
                iid = it.get(id_key)
                if iid is None:
                    continue
...
def item_detail_kb(uid: int, db: DB, type_: str, item: Dict, cat_id: str, page: int, xt: Xtream,
    ...
    """أزرار كارت العمل: تشغيل مباشر + مفضلة + رجوع — متناسقة."""
    rows = []
    id_key = "series_id" if type_ == "series" else "stream_id"
    iid = item.get(id_key)
    if type_ == "live":
        # تشغيل مباشر عبر رابط الويب الرسمي.
        rows.append([InlineKeyboardButton("▶️ مشاهدة مباشرة", url=player_link(xt.live_url(iid), title))])
    elif type_ == "movie":
...
class CinemaBot:
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if not cats:
            ...
            return
        # الفئات الرئيسية دائماً: لغة/منشأ للأفلام والمسلسلات، وكل باقة مستقلة للقنوات.
        groups = group_categories(cats, type_)
        GROUPS_CACHE[type_] = list(groups.items())
        await self._safe_edit(
            q,
            f"✨ <b>{TYPE_LABEL[type_]}</b> ✨\n━━━━━━━━━━━━━━━\n"
            f"📂 <b>{len(groups)} قسم رئيسي</b> — اختر القسم:",
            groups_kb(groups, type_, 0))

    async def _require_adult(self, q, uid: int) -> bool:
        """محتوى الكبار للمالك فقط؛ لا يتم طلب أو كشف كلمة المرور لأي مستخدم."""
        if is_admin(uid):
            return True
        await self._safe_edit(
            q,
            "🔞✨ <b>محتوى خاص</b> ✨\n━━━━━━━━━━━━━━━\n"
            "🔒 هذا القسم متاح لمالك البوت فقط.\n\n"
            "📩 إذا كنت تحتاج الوصول، تواصل مع مطور البوت.",
            InlineKeyboardMarkup([back_main_row()]))
        return False
    ...
    async def _show_items(self, q, type_: str, cat_id: str, page: int):
        ...
        if cat_id.startswith("g") and cat_id[1:].isdigit():
            ...
            if not is_admin(q.from_user.id):
                items = [it for it in items if not is_adult_name(it.get("name") or it.get("title") or "")]
        if not items:
            ...
            return
        # حماية قسم الكبار فقط؛ العناصر الفردية للكبار تُخفى عن المستخدم العادي داخل الأقسام المختلطة.
        header = group_label or TYPE_LABEL[type_]
        if is_adult_name(header):
            if not await self._require_adult(q, q.from_user.id):
                return
        ...
        try:
            ...
            if data == "main":
                ...
                await self._safe_edit(
                    ...
                    "🔗 <b>رابط مباشر:</b> أرسل أي رابط HTTP/HTTPS وسيتم تجهيز رابط تشغيل ويب مباشر.\n"
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await update.message.reply_text(f"🔍 جاري البحث عن: <b>{esc(text)}</b>...",
                                              parse_mode=ParseMode.HTML)
        results = await self.store.search(text, uid)
        if not results:
            await msg.edit_text(
    ...
    def run(self):
        ...
        log.info("🚀 %s يعمل الآن — السيرفر: %s | رابط التشغيل: %s", BOT_NAME, self.xt.base, PLAYER_URL)
