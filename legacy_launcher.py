...
# Support both the old single-admin variable and the newer comma-separated list.
try:
    admin_values = os.getenv("ADMIN_IDS", "").strip()
    if admin_values:
        parsed_admins = [int(x.strip()) for x in admin_values.replace(";", ",").split(",") if x.strip().isdigit()]
    else:
        admin_raw = os.getenv("ADMIN_ID", "").strip()
        parsed_admins = [int(admin_raw)] if admin_raw.isdigit() else []
    if parsed_admins:
        cinema.ADMIN_IDS = parsed_admins
        cinema.ADMIN_ID = parsed_admins[0]
        youseif.ADMIN_IDS = parsed_admins
        youseif.ADMIN_ID = parsed_admins[0]
except Exception:
    log.exception("Failed to load admin IDs")
...
def admin_bots_keyboard():
    from telegram import InlineKeyboardButton, InlineKeyboardMarkup
    rows=[]
    visible = sum(1 for pid in PLUGINS if pid in ENABLED_PLUGINS)
    rows.append([InlineKeyboardButton(f"👁️ الظاهر {visible}/{len(PLUGINS)} — اضغط للتبديل", callback_data="noop")])
    for pid, mod in PLUGINS.items():
        is_visible = pid in ENABLED_PLUGINS
        status = "🟢 ظاهر" if is_visible else "⚫ مخفي"
        rows.append([InlineKeyboardButton(f"{status} · {_plugin_button(mod, pid)}", callback_data=f"adm:toggle:{pid}")])
    rows.append([InlineKeyboardButton("🔄 إعادة تحميل المنصات", callback_data="adm:reload")])
...
async def handle_admin_callback(upd, uid, data):
    ...
    if data=="adm:bots":
        await PTB_BOT.edit_message_text(chat_id=chat_id,message_id=msg_id,text=f"🤖 <b>إظهار وإخفاء المنصات</b>\n━━━━━━━━━━━━━━━\n👁️ ظاهر: {sum(1 for p in PLUGINS if p in ENABLED_PLUGINS)}\n📦 الإجمالي: {len(PLUGINS)}\n\nاضغط على أي منصة لتبديل ظهورها في البوابة والبحث الموحد.\nالإخفاء لا يحذف المنصة ولا ملفاتها.",parse_mode=youseif.ParseMode.HTML,reply_markup=admin_bots_keyboard()); return True
    if data.startswith("adm:toggle:"):
        ...
        await PTB_BOT.edit_message_text(chat_id=chat_id,message_id=msg_id,text="👁️ <b>إظهار وإخفاء المنصات</b>\n\nاختر منصة لتبديل ظهورها:",parse_mode=youseif.ParseMode.HTML,reply_markup=admin_bots_keyboard()); return True
    if data=="adm:reload":
