...
class CinemaBot:
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if type_ == "live":
            ...
            rows = grid([InlineKeyboardButton(
                f"{g['icon']} {g['key']}",
                callback_data=f"hg:{type_}:{i}"
            ) for i, g in enumerate(hierarchy)], 2)
    ...
    async def _require_adult(self, q, uid: int) -> bool:
        """حماية محتوى الكبار: الأدمن أو من فتح بالباسورد مرة واحدة."""
        ...
        await self._safe_edit(
            ...
            "📩 اطلب الباسورد من مالك البوت ثم أرسله هنا مرة واحدة.",
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # 🔐 انتظار إدخال باسورد محتوى الكبار
        if uid in ADULT_PENDING:
            pending = ADULT_PENDING.get(uid) or {}
            expected = self.db.get_or_create_adult_password()
            if text.strip() == expected:
                self.db.unlock_adult(uid)
                ADULT_PENDING.pop(uid, None)
                await update.message.reply_text(
                    "✅ تم فتح قسم الكبار لهذا الحساب.\nيمكنك الرجوع والمتابعة.",
                    reply_markup=InlineKeyboardMarkup([back_main_row()]),
                )
                return
            ADULT_PENDING.pop(uid, None)
            await update.message.reply_text(
                "❌ الباسورد غير صحيح.\nاطلب الباسورد الصحيح من مالك البوت.",
                reply_markup=InlineKeyboardMarkup([back_main_row()]),
            )
            return
