...
## إعداد Telegram خطوة بخطوة

1. افتح `@BotFather` وأنشئ البوت عبر `/newbot`، ثم خزّن `BOT_TOKEN` في إعدادات التشغيل.
2. من `@BotFather` استخدم `/setdomain` وحدد نطاق Mini App العام، ثم عيّن رابط التطبيق عبر زر Web App في إعدادات البوت أو عبر `MINI_APP_URL` في بيئة التشغيل.
