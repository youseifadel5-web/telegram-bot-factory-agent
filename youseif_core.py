...
if not TMDB_READ_TOKEN:
    TMDB_READ_TOKEN = str(_c("API_Read_Access_Token", "")).strip()
...
def resolve_live_url(xt: "Xtream", item: Dict) -> str:
    """يفضل الرابط الخام القادم من المصدر، ثم يبني رابط Xtream القياسي فقط عند غيابه."""
    raw = item.get("stream_url") or item.get("direct_url") or item.get("url")
    return direct_url(raw) if raw else xt.live_url(item.get("stream_id"))


def resolve_movie_url(xt: "Xtream", item: Dict, ext: str = "mp4") -> str:
    raw = item.get("stream_url") or item.get("direct_url") or item.get("url")
    return direct_url(raw) if raw else xt.movie_url(item.get("stream_id"), ext)


def resolve_episode_url(xt: "Xtream", ep: Dict) -> str:
    raw = ep.get("stream_url") or ep.get("direct_url") or ep.get("url")
    if raw:
        return direct_url(raw)
    return xt.episode_url(ep.get("id"), ep.get("container_extension") or "mp4")
            ...
            try:
                ...
                if r.status_code == 200:
                    try:
                        ...
                        log.error("❌ استجابة غير صالحة من Xtream (%s): %s", action or "login", r.text[:300])
                        self._switch()
                        continue
                    if isinstance(data, dict) and data.get("error"):
                        last_err = str(data.get("error"))
                        log.error("❌ Xtream API رفض الطلب (%s): %s", action or "login", last_err)
                        self._switch()
                        continue
                    if data is None:
                        last_err = "empty JSON"
                        log.error("❌ Xtream API أعاد استجابة فارغة (%s)", action or "login")
                        self._switch()
                        continue
                    if isinstance(data, list) and not data:
                        log.warning("⚠️ Xtream action=%s returned empty list from %s", action or "login", self.base)
                    elif isinstance(data, dict) and not data:
                        log.warning("⚠️ Xtream action=%s returned empty object from %s", action or "login", self.base)
                    return data
                last_err = f"HTTP {r.status_code}"
                log.error("❌ Xtream HTTP %s (%s): %s", r.status_code, action or "login", r.text[:200])
            except Exception as e:
...
class Store:
    ...
    async def _cached(self, key: str, action: str, **params):
        ...
        if data is None:
            log.error("❌ لا توجد استجابة من المصدر: action=%s params=%s", action, params)
            return None
        if isinstance(data, (list, dict)) and not data:
            # لا نحفظ الفشل/الاستجابة الفارغة في الكاش؛ حتى لا تختفي البيانات لمدة CACHE_DURATION.
            log.warning("⚠️ المصدر أعاد بيانات فارغة: action=%s params=%s", action, params)
            return data
    ...
    async def categories(self, type_: str) -> List[Dict]:
        ...
        if not isinstance(data, list):
            log.error("❌ تصنيفات %s غير صالحة: %r", type_, type(data).__name__)
            return []
        log.info("📚 تم جلب %d تصنيفاً لـ %s", len(data), type_)
        return data
    ...
    async def streams(self, type_: str, cat_id=None) -> List[Dict]:
        ...
        if not isinstance(data, list):
            log.error("❌ عناصر %s غير صالحة: category=%s response=%s", type_, cat_id, type(data).__name__)
            return []
        log.info("📦 تم جلب %d عنصر لـ %s category=%s", len(data), type_, cat_id or "all")
        return data
    ...
    async def search(self, query: str) -> List[Tuple[str, Dict]]:
        ...
        for type_ in ("movie", "series", "live"):
            ...
            for it in items:
                ...
                haystack = " ".join([
                    name, name_clean,
                    normalize_ar(str(it.get("category_name") or "")),
                    normalize_ar(str(it.get("stream_type") or "")),
                    normalize_ar(str(it.get("plot") or "")),
                    normalize_ar(str(it.get("description") or "")),
                    normalize_ar(str(it.get("title") or "")),
                ])
...
def main_menu_kb(uid: int = 0) -> InlineKeyboardMarkup:
    """القائمة الرئيسية العلوية فقط — كل الخدمات السريعة Inline بدون لوحة سفلية."""
    rows = [
...
def back_main_row() -> List[InlineKeyboardButton]:
    """شريط تنقل ثابت: قائمة يوسف + بوابة اختيار البوتات."""
    return [
        InlineKeyboardButton("🏠 قائمة يوسف", callback_data="main"),
        InlineKeyboardButton("🎬 البوابة الرئيسية", callback_data="hub_home"),
    ]


def nav_footer(*extra_rows) -> List[List[InlineKeyboardButton]]:
    """أضف أزرار رجوع إضافية ثم الشريط الثابت."""
...
def normalize_poster_url(url: str, base: str = "") -> str:
    """وحّد رابط البوستر (نسبي → مطلق، http → https عند الإمكان)."""
    u = (url or "").strip()
    ...
    if u.startswith("//"):
        ...
        u = base.rstrip("/") + u
    elif not u.startswith(("http://", "https://")) and base:
        u = base.rstrip("/") + "/" + u.lstrip("/")
    # بعض السيرفرات تعطي روابط http غير آمنة — جرّب https أولاً في الإرسال
    return u
...
async def build_content_hierarchy(store, type_: str) -> List[Dict]:
    cats = await store.categories(type_)
    if not cats:
        log.error("❌ تعذر بناء هرم %s: قائمة التصنيفات فارغة", type_)
        return []
    # نعتمد أولاً على stream_count من السيرفر؛ نمسح all_items فقط لو الأعداد ناقصة بشكل كبير
    # (تجنب تأخير كبير على المكتبات الضخمة).
    counts = {}
    missing = 0
    for c in cats:
        sc = c.get("stream_count")
        cid = str(c.get("category_id") or "")
        if sc is not None and str(sc).isdigit():
            counts[cid] = int(sc)
        else:
            missing += 1
    if missing > max(3, len(cats) // 4):
        try:
            all_items = await store.all_items(type_)
            id_field = "category_id"
            for it in all_items:
                cid = str(it.get(id_field) or "")
                if cid:
                    counts[cid] = counts.get(cid, 0) + 1
        except Exception as e:
            log.exception("❌ فشل حساب أعداد %s: %s", type_, e)
    if type_ == "live":
        groups = {}
        for c in cats:
            ...
            if is_adult_name(raw) or genre == "للكبار فقط":
                genre, icon = "للكبار فقط", "🔞"
            ...
            if cid not in p["ids"]:
                p["ids"].append(cid)
            p["count"] += counts.get(cid, int(c.get("stream_count") or 0))
    ...
    for c in cats:
        ...
        if cid not in d["ids"]:
            d["ids"].append(cid)
        d["count"] += counts.get(cid, int(c.get("stream_count") or 0))
...
def item_detail_kb(uid: int, db: DB, type_: str, item: Dict, cat_id: str, page: int, xt: Xtream,
    ...
    if type_ == "live":
        ...
        rows.append([
            InlineKeyboardButton("▶️ مشاهدة", url=resolve_live_url(xt, item)),
            InlineKeyboardButton("📡 بث مباشر", url=resolve_live_url(xt, item)),
        ])
    elif type_ == "movie":
        ext = item.get("container_extension") or "mp4"
        rows.append([
            InlineKeyboardButton("▶️ مشاهدة", url=resolve_movie_url(xt, item, item.get("container_extension") or "mp4")),
            InlineKeyboardButton(f"⬇️ تحميل ({ext})", url=resolve_movie_url(xt, item, ext)),
        ])
    elif type_ == "series":
...
def seasons_kb(info: Dict, series_id) -> InlineKeyboardMarkup:
    ...
    rows = grid(btns, 2)
    rows.append([InlineKeyboardButton("🔙 تفاصيل المسلسل", callback_data=f"i:series:{series_id}:all:0")])
    rows.append(back_main_row())
    return InlineKeyboardMarkup(rows)
...
def episodes_kb(info: Dict, series_id, season: int, page: int) -> InlineKeyboardMarkup:
    ...
    if pages > 1:
        ...
        rows.append(nav)
    rows.append([
        InlineKeyboardButton("🔙 المواسم", callback_data=f"s:{series_id}:0"),
        InlineKeyboardButton("🔙 المسلسل", callback_data=f"i:series:{series_id}:all:0"),
    ])
    rows.append(back_main_row())
    return InlineKeyboardMarkup(rows)
...
def episode_play_kb(ep: Dict, series_id, season: int, page: int, xt: Xtream,
    ...
    """زر مشاهدة الحلقة عبر رابط المصدر المباشر + رجوع واضح."""
    eid = ep.get("id")
    num = ep.get("episode_num", "?")
...
class CinemaBot:
    ...
    async def _show_categories(self, q, type_: str):
        ...
        if type_ == "live":
            ...
            for i, g in enumerate(hierarchy):
                # زر الكبار يظهر للأدمن فقط
                if g.get("key") in ("تصنيف خاص", "للكبار فقط") and not is_admin(uid):
                    continue
                btns.append(InlineKeyboardButton(f"{g['icon']} {g['key']}", callback_data=f"lt:{i}"))
            rows = grid(btns, 2)
            ...
            for i, g in enumerate(hierarchy):
                if g.get("key") in ("تصنيف خاص", "للكبار فقط") and not is_admin(uid):
                    continue
                btns.append(InlineKeyboardButton(f"{g['icon']} {g['key']}", callback_data=f"hg:{type_}:{i}"))
            rows = grid(btns, 2)
    ...
    async def _show_genre_countries(self, q, type_: str, idx: int):
        ...
        if idx < 0 or idx >= len(hierarchy):
            return await self._show_categories(q, type_)
        g = hierarchy[idx]; entries = list(g["countries"].items())
        if g.get("key") in ("تصنيف خاص", "للكبار فقط"):
            if not await self._require_adult(q, q.from_user.id):
                ADULT_PENDING[q.from_user.id] = {
                    "type": type_, "cat_id": f"hg:{type_}:{idx}", "page": 0, "kind": "genre",
                }
                return
        # مجموعة واحدة فقط → افتح العناصر مباشرة بدون قائمة فرعية
        if len(entries) == 1:
            return await self._show_genre_items(q, type_, idx, 0, 0)
        rows = grid([InlineKeyboardButton(f"{n} ({d['count']:,})", callback_data=f"hc:{type_}:{idx}:{j}") for j, (n, d) in enumerate(entries)], 2)
        rows.append([InlineKeyboardButton("🔙 التصنيفات الرئيسية", callback_data=f"t:{type_}")])
        rows.append(back_main_row())
    ...
    async def _show_genre_items(self, q, type_: str, gidx: int, cidx: int, page: int = 0):
        hierarchy = HIER_CACHE.get(type_) or await build_content_hierarchy(self.store, type_); HIER_CACHE[type_] = hierarchy
        if gidx < 0 or gidx >= len(hierarchy):
            return await self._show_categories(q, type_)
        g = hierarchy[gidx]; entries = list(g["countries"].items())
        if g.get("key") in ("تصنيف خاص", "للكبار فقط"):
            if not await self._require_adult(q, q.from_user.id):
                ADULT_PENDING[q.from_user.id] = {
                    "type": type_, "cat_id": f"h:{gidx}:{cidx}", "page": page, "kind": "items",
                }
                return
        if cidx < 0 or cidx >= len(entries):
            return await self._show_genre_countries(q, type_, gidx)
    ...
    async def _show_live_packages(self, q, idx: int):
        ...
        if idx < 0 or idx >= len(hierarchy):
            return await self._show_categories(q, "live")
        g = hierarchy[idx]; entries = list(g["packages"].items())
        if g.get("key") in ("تصنيف خاص", "للكبار فقط"):
            if not await self._require_adult(q, q.from_user.id):
                ADULT_PENDING[q.from_user.id] = {
                    "type": "live", "cat_id": f"lt:{idx}", "page": 0, "kind": "packages",
                }
                return
        # باقة واحدة فقط → افتح القنوات مباشرة
        if len(entries) == 1:
            return await self._show_live_items(q, idx, 0, 0)
        rows = grid([InlineKeyboardButton(f"📦 {n} ({d['count']:,})", callback_data=f"lp:{idx}:{j}") for j, (n, d) in enumerate(entries)], 2)
        rows.append([InlineKeyboardButton("🔙 الأنواع الرئيسية", callback_data="t:live")])
        rows.append(back_main_row())
    ...
    async def _show_live_items(self, q, gidx: int, pidx: int, page: int = 0):
        ...
        if pidx < 0 or pidx >= len(entries):
            return await self._show_live_packages(q, gidx)
        pname, d = entries[pidx]; items = []; seen = set()
        adult_section = g.get("key") in ("تصنيف خاص", "للكبار فقط")
        if adult_section:
            if not await self._require_adult(q, q.from_user.id):
                ADULT_PENDING[q.from_user.id] = {
                    "type": "live", "cat_id": f"lp:{gidx}:{pidx}", "page": page, "kind": "items",
                }
                return
        for rid in d["ids"]:
            for it in await self.store.streams("live", rid):
                ...
                if iid is None or iid in seen:
                    continue
                if is_non_islamic_religious(name):
                    continue
                if not adult_section and is_adult_name(name):
                    continue
                seen.add(iid); items.append(it)
        if not items:
    ...
    async def _require_adult(self, q, uid: int) -> bool:
        """محتوى الكبار: للأدمن فقط — لا يظهر ولا يُفتح لأي مستخدم آخر."""
        if is_admin(uid):
            return True
    ...
    async def _show_items(self, q, type_: str, cat_id: str, page: int):
        # حماية قبل جلب العناصر: لا نسمح حتى بالوصول إلى تصنيف مقيد عبر callback متلاعب.
        if not is_admin(q.from_user.id) and not cat_id.startswith(("g", "h:", "lp:")):
            try:
                for cat in await self.store.categories(type_):
                    if str(cat.get("category_id")) == str(cat_id) and is_adult_name(cat.get("category_name") or ""):
                        await self._require_adult(q, q.from_user.id)
                        return
            except Exception as e:
                log.exception("❌ فشل فحص صلاحية التصنيف %s/%s: %s", type_, cat_id, e)
        ...
        if (is_adult_name(header) or any(is_adult_name(it.get("name") or "") for it in items[:5])) and not is_admin(q.from_user.id):
            if not await self._require_adult(q, q.from_user.id):
                ADULT_PENDING[q.from_user.id] = {
                    "type": type_, "cat_id": cat_id, "page": page, "kind": "items",
                }
    ...
    async def _find_item(self, type_: str, iid: str, cat_id: str = "") -> Optional[Dict]:
        """العثور على العنصر من نفس مصدر/تصنيف القائمة قبل محاولة all-items.
        بعض سيرفرات Xtream لا تعيد كل العناصر عند استدعاء endpoint بدون category_id،
        لذلك الاعتماد على streams(type_, None) وحده كان سبباً في ظهور "العنصر غير موجود"
        رغم أن اسمه ظهر بالفعل في القائمة.
        """
        id_key = "series_id" if type_ == "series" else "stream_id"
        target = str(iid)
        ...
        async def scan(ids):
            for cid in ids:
                try:
                    items = await self.store.streams(type_, cid)
                    for it in items:
                        if str(it.get(id_key)) == target:
                            return it
                except Exception as e:
                    log.exception("❌ فشل البحث عن العنصر %s/%s داخل التصنيف %s: %s", type_, iid, cid, e)
            return None

        async def scan_list(items):
            for it in items or []:
                if str(it.get(id_key)) == target:
                    return it
            return None

        # 1) مسار هرمي h:genre:country
        if cat_id.startswith("h:"):
            try:
                _, gi, ci = cat_id.split(":", 2)
                hierarchy = HIER_CACHE.get(type_) or await build_content_hierarchy(self.store, type_)
                if 0 <= int(gi) < len(hierarchy):
                    entries = list(hierarchy[int(gi)]["countries"].items())
                    if 0 <= int(ci) < len(entries):
                        found = await scan(entries[int(ci)][1]["ids"])
                        if found:
                            return found
            except Exception as e:
                log.exception("❌ فشل تحليل مسار العنصر h: %s", e)

        # 2) مسار باقات القنوات lp:genre:package
        if cat_id.startswith("lp:") and type_ == "live":
            try:
                _, gi, pi = cat_id.split(":", 2)
                hierarchy = HIER_CACHE.get("live") or await build_content_hierarchy(self.store, "live")
                if 0 <= int(gi) < len(hierarchy):
                    entries = list(hierarchy[int(gi)]["packages"].items())
                    if 0 <= int(pi) < len(entries):
                        found = await scan(entries[int(pi)][1]["ids"])
                        if found:
                            return found
            except Exception as e:
                log.exception("❌ فشل تحليل مسار القناة lp: %s", e)
        ...
        if cat_id.startswith("g") and cat_id[1:].isdigit():
            try:
                ...
                if 0 <= idx < len(entries):
                    found = await scan(entries[idx][1].get("ids") or [])
                    if found:
                        return found
            except Exception as e:
                log.exception("❌ فشل تحليل مسار المجموعة g: %s", e)

        # 4) تصنيف خام مباشر (رقم category_id) — الأهم لمنع البطء و"العنصر غير موجود"
        if cat_id and cat_id not in ("all", "fav", "") and not cat_id.startswith(("h:", "lp:", "g")):
            try:
                items = await self.store.streams(type_, cat_id)
                found = await scan_list(items)
                if found:
                    return found
            except Exception as e:
                log.exception("❌ فشل جلب التصنيف المباشر %s/%s: %s", type_, cat_id, e)

        # 5) fallback: all-items (قد يكون بطيئاً أو ناقصاً على بعض السيرفرات)
        try:
            ...
            log.exception("❌ فشل البحث العام عن العنصر %s/%s: %s", type_, iid, e)
        return None

    # ---------- كارت العمل (بوستر + TMDB + أزرار) ----------
    async def _show_item(self, q, uid: int, type_: str, iid: str, cat_id: str, page: int):
        item = await self._find_item(type_, iid, cat_id)
        if not item:
        ...
        if kind and not self.tmdb.enabled:
            log.warning("TMDB غير مفعّل — ضع TMDB_API_KEY في Secrets لظهور البوسترات")
        ...
        base = getattr(self.xt, "base", "") or ""
        poster_candidates = [
            meta.get("poster"),
            item.get("stream_icon"),
            item.get("cover"),
            item.get("cover_big"),
            item.get("movie_image"),
        ]
        ...
        if type_ == "series":
            ...
            poster_candidates.extend([
                ...
                series_info.get("cover_big"),
                series_info.get("movie_image"),
                series_info.get("stream_icon"),
            ])
            seasons = info.get("seasons") or []
            ...
            if not overview:
                overview = series_info.get("plot") or item.get("plot") or ""
        if type_ == "movie":
        ...
        for cand in poster_candidates:
            poster = normalize_poster_url(str(cand or ""), base)
            if poster:
                break
        ...
        if poster:
            try:
                ...
                pass
            sent = False
            for try_url in (poster, poster.replace("http://", "https://") if poster.startswith("http://") else None):
                if not try_url:
                    continue
                try:
                    await q.get_bot().send_photo(
                        q.message.chat_id, try_url,
                        caption=text[:1000], parse_mode=ParseMode.HTML, reply_markup=kb)
                    sent = True
                    break
                except Exception as e:
                    log.warning("فشل إرسال البوستر (%s): %s", try_url[:80], e)
            if sent:
                return
    ...
    async def _play_episode(self, q, episode_id: str, series_id: str, season: int, page: int):
        ...
        if not ep:
            await self._safe_edit(q, "❌ الحلقة غير موجودة.",
                                  InlineKeyboardMarkup([[InlineKeyboardButton("🔙 الحلقات", callback_data=f"e:{series_id}:{season}:{page}")], back_main_row()]))
            return
        series_info = info.get("info") or {}
        series_name = clean_name(series_info.get("name") or "")
        title = clean_name(str(ep.get("title") or f"الحلقة {ep.get('episode_num', '?')}"))
        ep_info = ep.get("info") if isinstance(ep.get("info"), dict) else {}
        dur = ep_info.get("duration")
        ...
        text = "\n".join(lines)
        kb = episode_play_kb(ep, series_id, season, page, self.xt, series_name)
        ...
        for cand in (
            ...
            if poster:
                break
        if not poster and series_name:
            meta = await self.tmdb.lookup(series_name, "series")
            poster = normalize_poster_url(meta.get("poster") or "", base)
        ...
        if poster:
            ...
            for try_url in (poster, poster.replace("http://", "https://") if poster.startswith("http://") else None):
                ...
                try:
                    ...
                    log.warning("فشل بوستر الحلقة: %s", e)
        await self._safe_edit(q, text, kb)

    async def _show_favs_message(self, update, uid: int):
        favs=self.db.get_favs(uid)
        if not favs:
            await update.message.reply_text("❤️✨ <b>مفضلتك فارغة</b> ✨",parse_mode=ParseMode.HTML,); return
    ...
    async def _show_random_message(self, update, uid: int):
        ...
        if not items: return await update.message.reply_text("❌ لا يوجد محتوى متاح حالياً.",)
    ...
    async def on_callback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "main":
            ...
            elif data.startswith("hg:"):
                # نوع المحتوى -> الدولة
                _, t, idx = data.split(":", 2)
                await self._show_genre_countries(q, t, int(idx))
            elif data.startswith("hc:"):
                # نوع المحتوى -> الدولة -> العناصر
                _, t, gidx, cidx = data.split(":", 3)
                await self._show_genre_items(q, t, int(gidx), int(cidx), 0)
            elif data.startswith("hi:"):
                # صفحات عناصر الدولة
                _, t, gidx, cidx, page = data.split(":", 4)
                await self._show_genre_items(q, t, int(gidx), int(cidx), int(page))
            elif data.startswith("lt:"):
                # القنوات: النوع -> الباقة
                _, idx = data.split(":", 1)
                await self._show_live_packages(q, int(idx))
            elif data.startswith("lp:"):
                # القنوات: الباقة -> القنوات
                _, gidx, pidx = data.split(":", 2)
                await self._show_live_items(q, int(gidx), int(pidx), 0)
            elif data.startswith("li:"):
                # صفحات قنوات الباقة
                _, gidx, pidx, page = data.split(":", 3)
                await self._show_live_items(q, int(gidx), int(pidx), int(page))
                ...
                # c:type:cat_id:page — cat_id قد يحتوي على ":" في مسارات نادرة
                parts = data.split(":")
                if len(parts) < 4:
                    await self._safe_edit(q, "❌ بيانات غير صالحة.", main_menu_kb(uid))
                else:
                    t = parts[1]
                    page = parts[-1]
                    cid = ":".join(parts[2:-1])
                ...
                # i:type:iid:cat_id:page — cat_id غالباً h:0:1 أو lp:0:0 فيحتوي على ":"
                parts = data.split(":")
                if len(parts) < 5:
                    await self._safe_edit(q, "❌ بيانات غير صالحة.", main_menu_kb(uid))
                else:
                ...
                # f:type:iid:cat_id:page — نفس مشكلة الـ colons
                parts = data.split(":")
                if len(parts) < 5:
                    await self._safe_edit(q, "❌ بيانات غير صالحة.", main_menu_kb(uid))
                else:
                    ...
                    page = parts[-1]
                    cid = ":".join(parts[3:-1])
                    item = await self._find_item(t, iid, cid)
    ...
    async def text_router(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # 🔐 محتوى الكبار للأدمن فقط — أي محاولة من مستخدم تُرفض
        if uid in ADULT_PENDING:
            ADULT_PENDING.pop(uid, None)
            await update.message.reply_text(
                "✨ هذا القسم مخصص لمالك البوت فقط ✨\n━━━━━━━━━━━━━━━\n"
                "🔒 لا يمكن للمستخدمين الوصول إلى محتوى الكبار.",
    ...
    async def _post_init(self, app):
        ...
        # فحص مصدر المحتوى عند التشغيل لتسجيل سبب القوائم الفارغة مبكراً.
        for t in ("live", "movie", "series"):
            try:
                cats = await self.store.categories(t)
                if cats:
                    log.info("✅ فحص المصدر: %s = %d تصنيف", t, len(cats))
                else:
                    log.error("🚨 فحص المصدر: %s = 0 تصنيفات — راجع بيانات IPTV/اتصال السيرفر", t)
            except Exception as e:
                log.exception("🚨 فشل فحص المصدر %s: %s", t, e)
