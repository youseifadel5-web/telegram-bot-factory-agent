...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━\n"
        "📚 مكتبة من كل المصادر — بدون تكرار\n"
        "🤖 مساعد AI لترشيحات حقيقية\n"
        "▶️ تشغيل بعد فحص المصدر والجودة\n\n"
        "اختر من القائمة:",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("play:start:"):
        ...
        for s in item.sources:
            if s.url:
                urls.append(s.url)
        # try plugin get_sources
        for sid in item.source_ids:
            p = plugins.get(sid)
            if p:
                try:
                    extra = await p.get_sources(item.id.split(":")[-1], item.type.value)
                    for e in extra or []:
                        if e.get("url"):
                            urls.append(e["url"])
                except Exception:
                    pass
        if not urls:
            # fallback message — open via legacy if available
            await safe_edit(
                q,
                "❌ لا يوجد رابط تشغيل مباشر متاح حاليًا لهذا العنصر من المصادر.\n"
                "جرّب من البوت الفرعي أو مصدر آخر.",
                kb.item_kb(item.id),
            )
            return
        best_status = None
...
async def handle_admin(q, uid: int, data: str):
    ...
    if data.startswith("adm:blocklist:"):
        ...
        for b in items:
            ...
            rows.append([
                InlineKeyboardButton(f"📋 نسخ {str(cid)[:12]}", callback_data="noop"),
...
def bind_legacy_plugins():
    ...
    try:
        ...
        if p and hasattr(p, "bind_store"):
            bot = yc.CinemaBot()
            p.bind_store(bot.store)
            log.info("Bound youseif store")
    except Exception:
        log.info("youseif_core not bound (optional)")
    try:
        import cinema_core as cc
        p = plugins.get("nova")
        if p and hasattr(p, "bind_api") and hasattr(cc, "api"):
            p.bind_api(cc.api)
            log.info("Bound nova api")
        p2 = plugins.get("orion")
        if p2 and hasattr(p2, "bind_vault") and hasattr(cc, "vault_api"):
            p2.bind_vault(cc.vault_api)
            log.info("Bound orion vault")
    except Exception:
        log.info("cinema_core not bound (optional)")
