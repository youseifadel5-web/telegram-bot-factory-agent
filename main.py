...
from bot.handlers import live as live_hub
from bot.handlers import sources as src_browse
...
def is_admin(uid: int) -> bool:
    # ADMIN_IDS فارغ = وضع التطوير → اللوحة تظهر للكل (حتى لا "تختفي" نهائيًا)
...
async def show_with_poster(q, uid: int, text: str, markup=None):
    """عرض التفاصيل ببوستر (صورة + اسم احترافي) مع الرجوع للنص إذا فشلت الصورة."""
    item = ustate(uid).get("current_item")
    poster = ""
    if item is not None:
        poster = (getattr(item, "poster", "") or "").strip()
    if poster:
        try:
...
def build_youseif_store(quiet: bool = True):
    """بناء مخزن يوسف فيلم من بيانات config (Xtream) بدون تشغيل بوت تيليجرام كامل."""
    try:
        ...
        d = yc.DB(DB_PATH)
        return yc.Store(xt, d)
    except Exception as e:
        if not quiet:
            log.info("build youseif store failed: %s", e)
        return None
...
def get_live_store():
    ...
    if store is None:
        store = build_youseif_store()
        if store is not None:
            p.bind_store(store)
    return store


def collect_play_urls(item) -> List[tuple]:
    """كل روابط التشغيل من كل المصادر المدمجة: (url, source_id, quality)."""
    urls: List[tuple] = []
    seen = set()

    def add(u, sid="", quality=""):
        u = (u or "").strip()
        if u and u.startswith("http") and u not in seen:
            seen.add(u)
            urls.append((u, sid, quality))

    for s in item.sources:
        add(getattr(s, "url", None), getattr(s, "source_id", ""), getattr(s, "quality", ""))
        ...
        if isinstance(extra, dict):
            for k in ("url", "link", "stream_url", "streamUrl", "play_url", "playUrl",
                ...
                add(extra.get(k), getattr(s, "source_id", ""), "")
    return urls


async def show_play_links(q, uid: int):
    """كل الروابط والجودات من كل المصادر — أزرار حقيقية قابلة للفتح (وليس زر رجوع)."""
    item = ustate(uid).get("current_item")
    if not item:
        await safe_edit(q, "❌ لا يوجد عمل محدد.", kb.back_home())
        return
    pairs = collect_play_urls(item)
        ...
        try:
            ...
            for e in extra or []:
                if isinstance(e, str):
                    ...
                    u = (e.get("url") or e.get("link") or e.get("stream_url") or e.get("m3u8_url") or "").strip()
                    if u:
                        pairs.append((u, sid, str(e.get("quality") or e.get("label") or "")))
        except Exception:
            log.exception("sources %s", sid)
    seen = set()
    uniq = []
    for u, sid, ql in pairs:
        if u not in seen:
            seen.add(u)
            uniq.append((u, sid, ql))
    if not uniq:
        await safe_edit(
            q,
            "❌ لا توجد روابط تشغيل متاحة حاليًا من أي مصدر.\n"
            "جرّب لاحقًا أو افتح قسم البوت الأصلي من القائمة الرئيسية.",
            kb.item_kb(item.id),
        )
        return
    rows = []
    for u, sid, ql in uniq[:12]:
        name = plugins.get(sid).name if sid in plugins.plugins else (sid or "رابط")
        label = f"▶️ {name}"
        if ql:
            label += f" · {ql}"
        rows.append([InlineKeyboardButton(label, url=u)])
    rows.append([InlineKeyboardButton("🎛️ فحص الجودات", callback_data=f"play:start:{item.id}")])
    rows.append([
        InlineKeyboardButton("⬅️ رجوع", callback_data="lib:item:x"),
        InlineKeyboardButton("🏠 الرئيسية", callback_data="home"),
    ])
    src_names = ", ".join(plugins.get(s).name if s in plugins.plugins else s for s in item.source_ids) or "—"
    await safe_edit(
        q,
        f"🎞️ <b>روابط التشغيل والجودات</b>\n<code>{_esc(item.title[:60])}</code>\n"
        f"📡 المصادر المدمجة: {src_names}\n\n"
        f"اضغط أي رابط لفتحه مباشرة في المشغل:",
        InlineKeyboardMarkup(rows),
    )


async def open_live(q, uid: int):
    store = get_live_store()
    if not store:
        await safe_edit(
            q,
            "❌ مصدر القنوات (يوسف فيلم) غير مربوط.\n"
            "تأكد من بيانات Xtream في config.py ثم أعد التشغيل.",
            kb.back_home(),
        )
        return
    await live_hub.genre_page(q, store)


SRC_LABELS = {"youseif": "بوت يوسف فيلم", "nova": "سينما نوفا", "orion": "أوريون بلس"}


async def handle_src(q, uid: int, data: str):
    """أقسام البوتات القديمة كاملة داخل المنصة الموحدة + زر العودة للمنصة."""
    parts = data.split(":")
    sid = parts[1]
    name = SRC_LABELS.get(sid, sid)
    if sid == "youseif" and len(parts) == 2:
        rows = [
            [InlineKeyboardButton("🎬 أفلام يوسف", callback_data="src:youseif:movie")],
            [InlineKeyboardButton("📺 مسلسلات يوسف", callback_data="src:youseif:series")],
            [InlineKeyboardButton("📡 قنوات مباشرة", callback_data="lib:live")],
            [InlineKeyboardButton("🏛️ المنصة الموحدة", callback_data="lib:home")],
        ]
        await safe_edit(
            q,
            f"📺 <b>{name}</b>\n"
            "هذا القسم داخل المنصة الموحدة — نفس مكتبة البوت القديم كاملة.\n"
            "اختر:",
            InlineKeyboardMarkup(rows),
        )
        return
    if sid == "youseif":
        typ = parts[2]
        items = await src_browse.browse(plugins.get("youseif"), typ)
        ustate(uid)["results"] = items
        ustate(uid)["page"] = 0
        label = {
            "movie": "🎬 بوت يوسف فيلم — أفلام",
            "series": "📺 بوت يوسف فيلم — مسلسلات",
            "live": "📡 بوت يوسف فيلم — قنوات",
        }.get(typ, name)
        await show_results(q, uid, label, 0, back="src:youseif")
        return
    items = await search_engine.search(SearchQuery(text="", limit=40), plugin_ids=[sid])
    ustate(uid)["results"] = items
    ustate(uid)["page"] = 0
    await show_results(q, uid, f"📚 {name} — الأحدث", 0, back="lib:home")


# ── Commands ──────────────────────────────────────────────
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ALWAYS open main home — never history / last results / previous page."""
    u = update.effective_user
    db.touch_user(u.id, u.username or "", u.first_name or "")
    st = ustate(u.id)
    # hard reset session navigation (keep only long-lived prefs if any)
    for k in list(st.keys()):
        if k not in ("prefs",):
            st.pop(k, None)
    await update.message.reply_text(
        "✨ <b>منصة المشاهدة الموحّدة</b> ✨\n"
        "━━━━━━━━━━━━━━━\n"
        "🏛️ المنصة الموحدة: مكتبة كل البوتات بدون تكرار\n"
        "📡 قنوات مباشرة بتصنيف تفصيلي (يوسف باي)\n"
        "📚 أقسام البوتات القديمة كاملة داخل المنصة\n"
        "🤖 مساعد AI · ▶️ تشغيل بجودات متعددة\n\n"
        "اختر من القائمة:",
        parse_mode=ParseMode.HTML,
        reply_markup=kb.main_menu(is_admin(u.id)),
    )
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """فتح لوحة التحكم مباشرة — /admin (حتى لا تختفي اللوحة نهائيًا)."""
    uid = update.effective_user.id
    if not is_admin(uid):
        await update.message.reply_text(
            "🚫 لوحة الإدارة للأدمن فقط.\n"
            "اضبط ADMIN_IDS في المتغيرات ثم أعد التشغيل، أو أرسل /admin من حساب الأدمن."
        )
        return
    await update.message.reply_text(
        "👑 <b>لوحة التحكم</b>",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "noop":
        return
    if data == "home":
        await safe_edit(q, "🏠 <b>الرئيسية</b>\nاختر:", kb.main_menu(is_admin(uid)))
        return
    if data in ("lib:home",):
        await safe_edit(q, "📚 <b>المكتبة</b>\nاختر القسم:", kb.main_menu(is_admin(uid)))
        return

    # live channels (تصنيف تفصيلي) — قبل البحث العادي حتى لا يظهر القسم فارغًا
    if data.startswith("lib:type:") and data.endswith("live"):
        return await open_live(q, uid)
    ...
    if data.startswith("live:"):
        store = get_live_store()
        if not store:
            await safe_edit(q, "❌ مصدر القنوات غير متاح.", kb.back_home())
            return
        await live_hub.route(q, store, data)
        return

    if data.startswith("src:"):
        await handle_src(q, uid, data)
        return

    if data.startswith("play:links:"):
        await show_play_links(q, uid)
        return
    ...
    if data.startswith("play:start:"):
        ...
        if not urls:
            await safe_edit(
                q,
                "⚠️ لا يوجد رابط مباشر قابل للفحص من كل المصادر.\n"
                "جرّب «🎞️ كل الروابط والجودات» لفتح الروابط الخام أو افتح قسم البوت الأصلي.",
                kb.item_kb(item.id),
            )
            return
        best_status = None
    ...
    if data.startswith("adm:"):
        if not is_admin(uid):
            await safe_edit(q, "🚫 للأدمن فقط.", kb.back_home())
            return
