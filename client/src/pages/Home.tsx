  ...
  return (
    <div className="app-shell" dir="rtl">
      ...
      <main className="page-content">
        ...
        <section className="section-block"><div className="section-heading"><div><span className="section-kicker">مختارات اليوم</span><h2>الأكثر مشاهدة الآن</h2></div><button className="text-link">عرض الكل <ChevronLeft size={16} /></button></div><div className="movie-grid">{filtered.slice(0, 4).map((movie) => <MovieCard movie={movie} key={movie.id} onOpen={() => setSelected(movie)} />)}</div></section>
        ...
        <section className="section-block"><div className="section-heading"><div><span className="section-kicker">تصفح حسب ذوقك</span><h2>وصل حديثاً</h2></div><div className="arrows"><button><ChevronRight size={18} /></button><button><ChevronLeft size={18} /></button></div></div><div className="movie-grid">{filtered.slice(4).map((movie) => <MovieCard movie={movie} key={movie.id} onOpen={() => setSelected(movie)} />)}</div></section>
      </main>
