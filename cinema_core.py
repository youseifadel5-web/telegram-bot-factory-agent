...
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN غير مضبوط. ضعه في متغيرات البيئة / GitHub Secrets.")
...
def get_hub_keyboard():
    ...
    markup.add(
        types.InlineKeyboardButton("🌟 سينما نوفا", callback_data="hub_nova"),
        types.InlineKeyboardButton("🎞️ أوريون بلس", callback_data="hub_orion"),
        types.InlineKeyboardButton("🎬 𝑌𝑜𝑢𝑠𝑒𝑖𝑓 𝐹𝑖𝑙𝑚𝑠", callback_data="hub_youseif"),
        types.InlineKeyboardButton("🔍 بحث شامل في المكتبتين", callback_data="unified_search"),
    )
...
def handle_vault_callbacks(call):
    ...
    if data in {"hub_home", "vault_home"}:
        ...
        safe_edit_or_send(chat_id, call.message.message_id, "<b>✨ اختر وجهتك السينمائية:</b>", get_hub_keyboard())
        return True
    if data == "hub_nova":
        bot.answer_callback_query(call.id)
...
def send_welcome(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "✨ أهلاً بك في عالم المشاهدة!", reply_markup=get_hub_keyboard())
...
def process_unified_search(message):
    ...
    if len(query) < 2:
        bot.send_message(message.chat.id, "❌ اكتب كلمتين على الأقل للبحث.", reply_markup=get_hub_keyboard())
        return
...
def handle_callbacks(call):
    ...
    # --- اقتراح عشوائي ---
        ...
        bot.answer_callback_query(call.id)
        safe_edit_or_send(chat_id, call.message.message_id, "<b>✨ اختر وجهتك السينمائية:</b>", reply_markup=get_hub_keyboard())
